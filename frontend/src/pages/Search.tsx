...
const Search = () => {
  ...
  const handleHotelTypesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedHotelTypes((prevHotelTypes) =>
       event.target.checked? [...prevHotelTypes, hotelType] : prevHotelTypes.filter((types) => types !== hotelType)
    )
  };
  
  const handleFacilitiesChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const facilitiesFilter = event.target.value;
    
    setSelectedFacilities((prevFacility) =>
       event.target.checked? [...prevFacility, facilitiesFilter] : prevFacility.filter((facility) => facility !== facilitiesFilter)
    )
  };
